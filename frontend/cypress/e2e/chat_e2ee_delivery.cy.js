...
const ensureFriendship = (sender, receiver) => {
  ...
  return loginViaApi(sender)
    .then(() => cy.request(`/api/v1/profiles/${receiver.username}`))
    .then((response) => {
      expect(response.status).to.eq(200);
      receiverId = Number(response.body.user.id);
      return cy.request({
        method: 'DELETE',
        url: `/api/v1/friendships/${receiverId}`,
...
const prepareBrowserDevice = (user) => (
  cy.visit('/login')
    .then(() => cy.clearChatE2eeState())
    .then(() => loginViaApi(user))
    .then(() => cy.visit('/chat'))
    .then(() => {
    ...
    .then((bootstrap) => {
      expect(bootstrap.current_device_id).to.be.a('string').and.not.be.empty;
      return cy.logout().then(() => bootstrap.current_device_id);
    })
);

const openChatAsPreparedUser = (user, deviceId) => (
  loginViaApi(user)
    .then(() => cy.visit('/chat'))
    .then(() => {
      cy.contains('.chat-pill', /^Live$/, { timeout: 30000 }).should('be.visible');
    ...
    .then((bootstrap) => {
      expect(bootstrap.current_device_id).to.eq(deviceId);
      return bootstrap;
    })
);
...
describe('Encrypted Chat Delivery', () => {
  ...
  before(() => {
    ...
    // Cypress cannot maintain two real browser device stores at once. We bootstrap
    // Bob only to learn the live recipient device ID, then keep Alice as the single
    // active browser context for the sender-side diagnostic.
    prepareBrowserDevice(bob).then((deviceId) => {
      bobDeviceId = deviceId;
    });

    prepareBrowserDevice(alice).then((deviceId) => {
      aliceDeviceId = deviceId;
    });
  });

  it('proves encrypted DM send writes device-scoped payloads and survives sender reload', { retries: 0 }, () => {
    let aliceSendBaseline = null;

    openChatAsPreparedUser(alice, aliceDeviceId);
    ...
    if (liveSpacetimeAssertionsEnabled) {
      cy.then(() => {
        ...
        return waitForLiveDbMessageState({
          ...
          expect(conversationRows).to.have.length(1);
    ...
    cy.reload();
    waitForChatDeviceReady(aliceDeviceId);
